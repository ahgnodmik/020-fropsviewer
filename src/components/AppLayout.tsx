...
/**
 * PC: 좌우 사이드바(300×250) + 중앙 콘텐츠
 * 모바일: 콘텐츠 + 하단 고정 배너(320×50)
...
export function AppLayout({ children, fullHeight }: AppLayoutProps) {
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[var(--color-bg)]">
      {/* PC: 좌 사이드바 (md 이상에서만) */}
      <aside className="hidden md:flex flex-shrink-0 w-[300px] justify-center py-4 px-2 border-r border-[var(--color-border)] bg-[var(--color-surface)]">
      ...
      {/* PC: 우 사이드바 */}
      <aside className="hidden md:flex flex-shrink-0 w-[300px] justify-center py-4 px-2 border-l border-[var(--color-border)] bg-[var(--color-surface)]">
        <div className="sticky top-4">
          <AdSlot variant="sidebar" />
        </div>
      </aside>
